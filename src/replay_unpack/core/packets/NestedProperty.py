...
class NestedProperty(PrettyPrintObjectMixin):
    ...
    def read_and_apply(self, entity):
        ...
        while bit_reader.get(1) and obj:
            length = len(obj.client_properties) if isinstance(obj, Entity) else len(obj)
            max_bits = BitReader.bits_required(length)
            property_id = bit_reader.get(max_bits)
            if hasattr(obj, "get_field_name_for_index"):
                name = obj.get_field_name_for_index(property_id)  # type: ignore
                obj = obj[name]  # type: ignore
        ...
        if isinstance(obj, PyFixedDict):
            ...
            obj[field] = obj.get_field_type_for_index(index1).create_from_stream(
                BytesIO(bit_reader.get_rest())
            )
            ...
            if not rest:
                ...
                if self.is_slice:
                    logging.debug("removing element %s", obj[index1:index2])  # type: ignore
                    obj[index1:index2] = []  # type: ignore
                else:
                    obj[index1] = None
                return
            io = BytesIO(rest)
            ...
            if self.is_slice:
                logging.debug("replacing %s:%s with %s", index1, index2, new_elements)  # type: ignore
                obj[index1:index2] = new_elements  # type: ignore
            else:
