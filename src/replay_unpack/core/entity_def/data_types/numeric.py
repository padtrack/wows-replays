...
class _NumericType(DataType):
    ...
    def __init__(self, header_size=1):
        assert None not in [
            self.STRUCT_TYPE,
            self.PYTHON_TYPE,
        ], "STRUCT_TYPE and PYTHON_TYPE must be defined before initialization"
    ...
    def _get_default_value_from_section(self, section: etree.ElementBase):
        assert self.PYTHON_TYPE is not None, "PYTHON_TYPE undefined while getting default value"
