...
class _DataType(DataType):
    ...
    def _get_default_value_from_section(self, section: etree.ElementBase):
        raise RuntimeError(
            "_get_default_value_from_section for "
...
class FixedDict(_DataType):
    ...
    def from_section(cls, alias, section: etree.ElementBase, header_size=1):
        ...
        props: Iterable[etree.ElementBase] = section.find("Properties", namespaces=None)
        for prop in props:
            attributes[prop.tag] = alias.get_data_type_from_section(
                prop.find("Type", namespaces=None)
            )
        none_section = section.find("AllowNone", namespaces=None)
    ...
    def get_size_in_bytes(self):
        ...
        for value in self.attributes.values():
            size += value.get_size_in_bytes()  # type: ignore
        return size

    def __repr__(self):
        return "<FixedDict> {}".format({k: v for (k, v) in self.attributes.items()})
...
class Array(_DataType):
    ...
    def _get_value_from_stream(self, stream: BytesIO, header_size: int):
        ...
        if self.array_size is None:
            size = UInt8(header_size=header_size).create_from_stream(
    ...
    def from_section(cls, alias, section: etree.ElementBase, header_size):
        child_type = alias.get_data_type_from_section(section.find("of", namespaces=None))
        none_section = section.find("AllowNone", namespaces=None)
        allow_none = none_section is not None and none_section.text.strip() == "true"
        size_section = section.find("size", namespaces=None)
    ...
    def get_size_in_bytes(self):
        if self.array_size is not None:
            type_size = self.type.get_size_in_bytes()
            assert type_size is not None
            return type_size * self.array_size
        return INFINITY
    ...
    def _get_default_value_from_section(self, section: etree.ElementBase):
        ...
        """
        defaults = []
        for it in section.findall("item", namespaces=None):
...
class UserType(_DataType):
    ...
    @classmethod
    def from_section(cls, alias, section: etree.ElementBase, header_size):
        type_section = section.find("Type", namespaces=None)
