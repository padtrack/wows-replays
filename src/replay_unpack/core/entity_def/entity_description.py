...
class EntityMethod:
    def __init__(
        self, name: str, arguments: List[MethodArgument], header_size: int = DEFAULT_HEADER_SIZE
    ):
        self._name = name
    ...
    def get_size_in_bytes(self):
        size = sum(i.type.get_size_in_bytes() for i in self._arguments)  # type: ignore
        if size >= INFINITY:
    ...
    def _get_header_size_from_section(self, section: etree.ElementBase):
        ...
        """
        if section.find("VariableLengthHeaderSize", namespaces=None):
            return int(section.find("VariableLengthHeaderSize", namespaces=None).text.strip())
        return None

    @classmethod
    def from_section(cls, section: etree.ElementBase, alias):
        args = []
        if section.find("Args", namespaces=None) is not None:
            for item in section.find("Args", namespaces=None):
                args.append(
                    MethodArgument(name=item.tag, type_=alias.get_data_type_from_section(item))
                )
        else:
            for item in section.findall("Arg", namespaces=None):
                args.append(MethodArgument(type_=alias.get_data_type_from_section(item)))

        header_section = section.find("VariableLengthHeaderSize", namespaces=None)
        try:
    ...
    def __repr__(self):
        return "{name} ({args})".format(name=self._name, args=self._arguments)


class MethodDescriptions:
    def __init__(self):
        self._internal_index = []
        self._methods_by_name: Dict[str, EntityMethod] = {}

    def parse(self, section: etree.ElementBase, alias):
        for method in section.iterchildren(tag=None):
...
class EntityDef(BaseDataObjectDef):
    ...
    def _parse_section(self, section: etree.ElementBase):
        super()._parse_section(section)
        self._parse_client_methods(section.find("ClientMethods", namespaces=None))
        self._parse_cell_methods(section.find("CellMethods", namespaces=None))
        self._parse_base_methods(section.find("BaseMethods", namespaces=None))
