...
class BaseDataObjectDef:
    ...
    def _parse_implements(self, implements_list: etree.ElementBase):
        ...
        for it in implements_list.iterchildren(tag=None):
            path = os.path.join(
                self._base_dir, ENTITIES_DEFS_PATH, "interfaces", it.text.strip() + ".def"
            )
            parsed = etree.parse(
                path,
    ...
    def _parse_volatile(self, props_list: etree.ElementBase):
        ...
        for item in props_list.iterchildren(tag=None):
    ...
    def _parse_section(self, section: etree.ElementBase):
        self._parse_implements(section.find("Implements", namespaces=None))
        self._parse_properties(section.find("Properties", namespaces=None))
        self._parse_volatile(section.find("Volatile", namespaces=None))


class Property:
    def __init__(
        self, name: str, type_: DataType, flags: str, default: Optional[etree.ElementBase] = None
    ):
        self._name = name
        self._type = type_
        self._default = type_.get_default_value(default)  # type: ignore
    ...
    def get_size_in_bytes(self):
        return min(self._type.get_size_in_bytes(), INFINITY)  # type: ignore
    ...
    def from_section(cls, section: etree.ElementBase, alias):
        ...
        type_ = alias.get_data_type_from_section(section.find("Type", namespaces=None))
        default = section.find("Default", namespaces=None)
        flags = section.find("Flags", namespaces=None).text.strip()
    ...
    def __repr__(self):
        return "{name} ({args})".format(name=self._name, args=self._type)
...
class PropertiesDescriptions:
    ...
    def parse(self, section: etree.ElementBase, alias) -> None:
        for prop in section.iterchildren(tag=None):
